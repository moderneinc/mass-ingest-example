...
`publish.sh` is shared across all platforms — it's already platform-agnostic. Only the IaC and `chunk.sh` vary per cloud.
...
└── azure-batch/
    ...
    ├── chunk.sh                       # Uses `az batch task create`
    └── terraform/
        ├── main.tf
        ├── variables.tf
        └── terraform.tfvars.example
...
No functional changes. Content moves from `3-scalability/` into `3-scalability/aws-batch/`.
...
| Component | Implementation |
|---|---|
| Compute | Compute Engine VMs (e2-standard-4), allocation policy |
| Job orchestration | Task groups, `BATCH_TASK_INDEX` for partitioning |
| Scheduling | Cloud Scheduler (HTTP trigger to submit job) |
...
| Storage | GCS via S3-compatible interop API, or Maven/Artifactory |
| Volumes | Persistent Disk (pd-ssd) |

**Terraform resources**: `google_cloud_batch_job` (template), `google_service_account`, `google_secret_manager_secret`, `google_cloud_scheduler_job`, `google_compute_network`/`google_compute_firewall`, IAM bindings.
...
**Storage options**:
- GCS: Use the S3-compatible interop endpoint (`storage.googleapis.com`). `publish.sh` uses `s3://` URL with `S3_ENDPOINT` pointing to GCS interop.
- Maven/Artifactory: Works as-is with `PUBLISH_URL`, `PUBLISH_USER`, `PUBLISH_PASSWORD`/`PUBLISH_TOKEN`.
...
| Scheduling | Azure Logic Apps or Azure Automation (cron trigger) |
...
**Terraform resources**: `azurerm_batch_account`, `azurerm_batch_pool` (auto-scale formula), `azurerm_batch_job`, `azurerm_key_vault`, `azurerm_key_vault_secret`, `azurerm_logic_app_workflow` (scheduler), `azurerm_user_assigned_identity`, RBAC assignments, VNet/NSG.

**chunk.sh**: Uses `az batch task create` to submit N tasks to the pool. Each task gets a sequential ID (0, 1, 2, ...) used to derive `--start`/`--end`.
...
- `publish.sh` — already platform-agnostic via `--start`/`--end` args
...
2. Update any internal references/paths in the moved files
3. Create `3-scalability/README.md` (platform overview)
4. Create `3-scalability/gcp-batch/` with Terraform, chunk.sh, README
5. Create `3-scalability/azure-batch/` with Terraform, chunk.sh, README
6. Update root `README.md` to reflect the new structure

## Open questions

- **GCS S3 interop**: Need to verify that the Moderne CLI / `publish.sh` works correctly with GCS's S3-compatible endpoint. If not, Maven/Artifactory becomes the only GCS option.
- **Azure Batch container support**: Azure Batch supports running containers on pool nodes. Need to confirm the exact container configuration syntax for Terraform.
- **Azure scheduling**: Logic Apps vs Azure Automation vs Azure Functions timer trigger — need to pick the simplest option that can trigger a Batch job.
